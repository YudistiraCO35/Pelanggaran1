...
//  Untuk mengupgrade database jika ada perubahan di schema databasenya
    ...
    // Untuk menambahkan atau memperbarui data dalam database
    void tambahData(String nama, String kelas, int poin) {
        SQLiteDatabase db = this.getWritableDatabase();

        // Periksa apakah data dengan nama dan kelas yang sama sudah ada
        String query = "SELECT " + COLUMN_POIN + " FROM " + TABLE_NAME +
                " WHERE " + COLUMN_NAMA + " = ? AND " + COLUMN_KELAS + " = ?";
        Cursor cursor = db.rawQuery(query, new String[]{nama, kelas});

        if (cursor.moveToFirst()) {
            // Jika data sudah ada, tambahkan poin ke poin yang ada
            int currentPoin = cursor.getInt(0);
            int newPoin = currentPoin + poin;

            ContentValues cv = new ContentValues();
            cv.put(COLUMN_POIN, newPoin);

            // Perbarui data
            db.update(TABLE_NAME, cv, COLUMN_NAMA + " = ? AND " + COLUMN_KELAS + " = ?",
                    new String[]{nama, kelas});
        } else {
